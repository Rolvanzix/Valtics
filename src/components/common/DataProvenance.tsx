...
import { ShieldCheck, UserCheck, Calculator, AlertCircle, Info } from 'lucide-react';

export type ProvenanceType = 'on-chain' | 'issuer' | 'calculated' | 'unavailable';
...
export const ProvenanceBadge: React.FC<ProvenanceBadgeProps> = ({
  ...
  const configs: Record<ProvenanceType, { text: string; bg: string; textCol: string; border: string; icon: React.ReactNode; tooltip: string }> = {
    ...
    'calculated': {
      ...
      tooltip: 'Derived mathematically from reserves and bonding curve formula',
    },
    'unavailable': {
      text: label || 'Unavailable (No Indexer)',
      bg: 'bg-zinc-900',
      textCol: 'text-zinc-400',
      border: 'border-zinc-800',
      icon: <AlertCircle className="w-3 h-3 text-zinc-500 shrink-0" />,
      tooltip: 'Historical aggregate data is not indexed on this cluster',
    },
  };
  ...
  return (
    <div className={`p-3 rounded-lg bg-[#080b12] border border-zinc-800/80 space-y-2 text-[11px] ${className}`}>
      <div className="flex items-center gap-1.5 font-semibold text-zinc-300 font-sans">
        ...
        <span>Institutional Data Provenance Standard</span>
      </div>
      <p className="text-zinc-400 text-[10px] leading-relaxed">
        VALTICS strictly enforces truth in financial infrastructure. Numbers are never fabricated. Every datapoint is explicitly classified:
      </p>
      <div className="flex flex-wrap items-center gap-3 pt-1">
        ...
        <div className="flex items-center gap-1.5">
          ...
          <span className="text-zinc-400 text-[10px]">Pending off-chain indexer synchronization</span>
        </div>
      </div>
    </div>
  );
